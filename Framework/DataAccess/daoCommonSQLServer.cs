...
{
    public class daoCommonSQLServer : CBaseConnect
    {

        #region 初始化处理

        #region 变量定义
        //private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(daoCommonSQLServer));
        #endregion
                                ...
                                bool readUncommitted)
        {
            ...
            try
            {
                ...
                string strSql = (readUncommitted ? "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;" : "")
                       + " SELECT  CAST('0' AS Bit) AS SlctValue," + TableName + ".* "
                       + " FROM " + TableName
                       + " WHERE 1=1 ";
                ...
                foreach (string strKey in dicConds.Keys)
                {
                    if (dicItemData.ContainsKey(strKey) == true)
                    {
                        ...
                        SqlParameter = new SqlParameter("@" + strKey, Common._dicFiledTypeSQLServer[strKey]);

                        if (SqlParameter.SqlDbType == SqlDbType.Bit)
                        {
                            SqlParameter.Value = Common.GetBitValue(dicItemData[strKey]);
                        }
                        else
                        {
                                ...
                                bool readUncommitted)
        {
            ...
            try
            {
                ...
                string strSql = (readUncommitted ? "SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;" : "") + " SELECT  CAST('0' AS Bit) AS SlctValue," + TableName + ".* "
                             + " FROM " + TableName
                             + " WHERE 1=1 ";
                ...
                foreach (string strKey in dicBetweenConds.Keys)
                {
                        ...
                        && dicItemData.ContainsKey(strKey + Common.DefineValue.FiledEnd) == true)
                    {
                        ...
                        SqlParameter = new SqlParameter("@" + strKey + Common.DefineValue.FiledBegin, Common._dicFiledTypeSQLServer[strKey]);
                        SqlParameter.Value = Common.GetBeginTimeBySql(dicItemData[strKey + Common.DefineValue.FiledBegin]);
                        SqlParameter.Direction = ParameterDirection.Input;
                        cmdParamters[rowIndex] = SqlParameter;
                        rowIndex++;


                        SqlParameter = new SqlParameter("@" + strKey + Common.DefineValue.FiledEnd, Common._dicFiledTypeSQLServer[strKey]);
                        //获取结束时间处理
                        SqlParameter.Value = Common.GetEndTimeBySql(dicItemData[strKey + Common.DefineValue.FiledEnd]);
                        SqlParameter.Direction = ParameterDirection.Input;
                        cmdParamters[rowIndex] = SqlParameter;
                        rowIndex++;
                    }
                                ...
                                Common.DataModifyMode ScanMode)
        {
            ...
            try
            {
                ...
                if (dicItemData.ContainsKey(strRepFiledName) == true)
                {
                    ...
                    SqlParameter = new SqlParameter("@" + strRepFiledName, Common._dicFiledTypeSQLServer[strRepFiledName]);
